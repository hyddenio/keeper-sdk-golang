...
type authContext struct {
	...
	accountUid             []byte
	sessionToken           []byte
	sessionRestriction     auth.SessionRestriction
	dataKey                []byte
	ssoLoginInfo           auth.ISsoLoginInfo
	deviceToken            []byte
...
func (ac *authContext) AccountUid() []byte {
	return ac.accountUid
}
func (ac *authContext) SessionToken() []byte {
	return ac.sessionToken
}
func (ac *authContext) SessionRestriction() auth.SessionRestriction {
	return ac.sessionRestriction
}
func (ac *authContext) DataKey() []byte {
	return ac.dataKey
}
func (ac *authContext) SsoLoginInfo() auth.ISsoLoginInfo {
...
func newTimeToKeepalive(ac auth.IAuthContext) *timeToKeepalive {
...
func (ka *keeperAuth) PushNotifications() auth.IPushEndpoint {
	return ka.pushNotifications
}

func (ka *keeperAuth) Endpoint() auth.IKeeperEndpoint {
	return ka.endpoint
}
func (ka *keeperAuth) AuthContext() auth.IAuthContext {
	return ka.authContext
}
func (ka *keeperAuth) executeV2Command(request auth.IKeeperCommand, response auth.IKeeperResponse) (err error) {
...
func (ka *keeperAuth) ExecuteAuthCommand(request auth.IKeeperCommand, response auth.IKeeperResponse, throwOnError bool) (err error) {
...
func (ka *keeperAuth) ExecuteBatch(requests []auth.IKeeperCommand) (responses []*auth.KeeperApiResponse, err error) {
	var logger = api.GetLogger()
