...
var (
	_ auth.IDeviceApprovalStep   = &deviceApprovalStep{}
	_ auth.ISsoLoginInfo         = &ssoLoginInfo{}
	_ auth.IAuthContext          = &authContext{}
	_ auth.IConnectedStep        = &connectedStep{}
	_ auth.ISsoTokenStep         = &ssoTokenStep{}
	_ auth.ITwoFactorStep        = &twoFactorStep{}
	_ auth.ITwoFactorChannelInfo = &twoFactorChannelInfo{}
	_ auth.IPasswordStep         = &passwordStep{}
	_ auth.ISsoDataKeyStep       = &ssoDataKeyShareStep{}
)
...
func (gs *genericAuthStep) LoginState() auth.LoginState {
	return gs.loginState
}

type deviceApprovalStep struct {
	genericAuthStep
	onSendPush func(auth.DeviceApprovalChannel) error
	onSendCode func(auth.DeviceApprovalChannel, string) error
	onResume   func() error
}

func (das *deviceApprovalStep) SendPush(channel auth.DeviceApprovalChannel) error {
	return das.onSendPush(channel)
}
func (das *deviceApprovalStep) SendCode(channel auth.DeviceApprovalChannel, code string) error {
...
type twoFactorChannelInfo struct {
	channelType auth.TwoFactorChannel
	channelName string
	channelUid  []byte
	phone       string
	pushActions []auth.TwoFactorPushAction
	challenge   string
	maxDuration auth.TwoFactorDuration
}

func (tci *twoFactorChannelInfo) MaxDuration() auth.TwoFactorDuration {
	return tci.maxDuration
}
func (tci *twoFactorChannelInfo) ChannelType() auth.TwoFactorChannel {
...
func (tci *twoFactorChannelInfo) PushActions() []auth.TwoFactorPushAction {
	return tci.pushActions[:]
}

type twoFactorStep struct {
	genericAuthStep
	duration   auth.TwoFactorDuration
	channels   []auth.ITwoFactorChannelInfo
	onSendPush func([]byte, auth.TwoFactorPushAction) error
	onSendCode func([]byte, string) error
	onResume   func() error
}

func (tf *twoFactorStep) Duration() auth.TwoFactorDuration {
	return tf.duration
}
func (tf *twoFactorStep) SetDuration(duration auth.TwoFactorDuration) {
	tf.duration = duration
}
func (tf *twoFactorStep) Channels() []auth.ITwoFactorChannelInfo {
	return tf.channels[:]
}
func (tf *twoFactorStep) SendPush(channelUid []byte, action auth.TwoFactorPushAction) error {
...
func newReadyStep() auth.ILoginStep {
	return &genericAuthStep{
		loginState: auth.LoginState_Ready,
	}
}
...
func newErrorStep(err error) auth.IErrorStep {
	return &errorStep{
		genericAuthStep: genericAuthStep{
			loginState: auth.LoginState_Error,
		},
		err: err,
	}
}
...
func (cs *connectedStep) TakeKeeperAuth() (result auth.IKeeperAuth, err error) {
...
type ssoDataKeyShareStep struct {
	genericAuthStep
	onRequestDataKey func(auth.DataKeyShareChannel) error
	onResume         func() error
}

func (dkss *ssoDataKeyShareStep) Channels() []auth.DataKeyShareChannel {
	return []auth.DataKeyShareChannel{
		auth.DataKeyShare_KeeperPush,
		auth.DataKeyShare_AdminApproval,
	}
}
func (dkss *ssoDataKeyShareStep) RequestDataKey(channel auth.DataKeyShareChannel) error {
...
func (la *loginAuth) onRequiresAuthHash(response *proto_auth.LoginResponse) (err error) {
	...
	var ps = &passwordStep{
		genericAuthStep: genericAuthStep{
			loginState: auth.LoginState_Password,
		},
...
func (la *loginAuth) onRequires2fa(response *proto_auth.LoginResponse) error {
	...
	var tfs = &twoFactorStep{
		genericAuthStep: genericAuthStep{
			loginState: auth.LoginState_TwoFactor,
		},
		duration: auth.TwoFactorDuration_EveryLogin,
	}
	for _, ch := range response.Channels {
		...
		case auth.TwoFactorChannel_TextMessage:
			ti.pushActions = []auth.TwoFactorPushAction{auth.TwoFactorAction_TextMessage}
		case auth.TwoFactorChannel_KeeperDNA:
			ti.pushActions = []auth.TwoFactorPushAction{auth.TwoFactorAction_KeeperDna}
		case auth.TwoFactorChannel_DuoSecurity:
			for _, x := range ch.GetCapabilities() {
				switch x {
				case "push":
					ti.pushActions = append(ti.pushActions, auth.TwoFactorAction_DuoPush)
				case "sms":
					ti.pushActions = append(ti.pushActions, auth.TwoFactorAction_DuoTextMessage)
				case "phone":
					ti.pushActions = append(ti.pushActions, auth.TwoFactorAction_DuoVoiceCall)
				}
			}
		}
		tfs.channels = append(tfs.channels, ti)
	}
	tfs.onSendPush = func(channelUid []byte, action auth.TwoFactorPushAction) (er1 error) {
	...
	tfs.onSendCode = func(channelUid []byte, code string) (er1 error) {
		...
		for _, ch := range tfs.channels {
			if bytes.Compare(ch.ChannelUid(), channelUid) == 0 {
	...
	var handler = func(event *auth.NotificationEvent) bool {
...
func (la *loginAuth) onDeviceApprovalRequired(response *proto_auth.LoginResponse) (err error) {
	...
	var das = &deviceApprovalStep{
		genericAuthStep: genericAuthStep{
			loginState: auth.LoginState_DeviceApproval,
			OnClose: func() error {
				if la.pushNotifications != nil {
					la.pushNotifications.RemoveAllCallback()
				}
		...
		onSendPush: func(channel auth.DeviceApprovalChannel) (er1 error) {
			if channel == auth.DeviceApproval_Email {
				...
				if channel == auth.DeviceApproval_KeeperPush {
		...
		onSendCode: func(channel auth.DeviceApprovalChannel, code string) (er1 error) {
			...
			} else if channel == auth.DeviceApproval_TwoFactorAuth {
...
func (la *loginAuth) onRequestDataKey(loginToken []byte) (err error) {
	...
	var dkss = &ssoDataKeyShareStep{
		genericAuthStep: genericAuthStep{
			loginState: auth.LoginState_SsoDataKey,
			OnClose: func() error {
				if la.pushNotifications != nil {
					la.pushNotifications.RemoveAllCallback()
				}
		...
		onRequestDataKey: func(channel auth.DataKeyShareChannel) (er1 error) {
			switch channel {
			case auth.DataKeyShare_KeeperPush:
				var rqPush = &proto_auth.TwoFactorSendPushRequest{
					EncryptedLoginToken: loginToken,
					PushType:            proto_auth.TwoFactorPushType_TWO_FA_PUSH_KEEPER,
				}
				er1 = la.executeRest("authentication/2fa_send_push", rqPush, nil, nil)
			case auth.DataKeyShare_AdminApproval:
	...
	var handle = func(event *auth.NotificationEvent) bool {
		var token []byte
...
func (la *loginAuth) onSsoRedirect(loginToken []byte) (err error) {
	...
	var sts = &ssoTokenStep{
		genericAuthStep: genericAuthStep{
			loginState: auth.LoginState_SsoToken,
		},
	...
	if uri, err = url.Parse(ssoInfo.SsoUrl()); err == nil {
		if sts.IsCloudSso() {
			...
			if data, err = proto.Marshal(rq); err == nil {
				...
				if apiRq, err = PrepareApiRequest(keyId, data, transmissionKey, nil, locale); err == nil {
					if data, err = proto.Marshal(apiRq); err == nil {
						uri.Query().Add("payload", api.Base64UrlEncode(data))
						...
						sts.onLoginWithPassword = func() error {
							...
							la.loginContext.AccountType = auth.AuthType_Regular
			...
			if rsaPrivate, rsaPublic, err = api.GenerateRsaKey(); err == nil {
				uri.Query().Add("key", api.Base64UrlEncode(api.UnloadRsaPublicKey(rsaPublic)))
				uri.Query().Add("embedded", "")
				sts.ssoLoginUrl = uri.String()
				sts.onSetSsoToken = func(token string) (er1 error) {
					var st = new(json_commands.SsoToken)
...
func (la *loginAuth) onLoggedIn(response *proto_auth.LoginResponse, decryptor func([]byte) ([]byte, error)) (err error) {
	...
	if ac.SessionRestriction() == auth.SessionRestriction_Unrestricted {
		la.ensurePushNotifications()
	...
	var cs = &connectedStep{
		genericAuthStep: genericAuthStep{
			loginState: auth.LoginState_Connected,
		},
		keeperAuth: &keeperAuth{
