...
func (s *sqlEntityStorage) GetEntity(uid string) (result sdk.IUid) {
	...
	if stmt, err = s.db.GetStatement(query); err == nil {
		...
		if scanner, err = GetScanner(value.Elem(), keyScanner, dataScanner); err == nil {
			if err = row.Scan(scanner...); err == nil {
				result, _ = value.Interface().(sdk.IUid)
			}
		}
...
func (s *sqlEntityStorage) PutEntity(entity sdk.IUid) {
...
func (s *sqlEntityStorage)EnumerateEntities(callback func (sdk.IUid) bool) {
			...
			for rows.Next() {
				...
				if values, err = GetScanner(value.Elem(), keyValues, dataValues); err == nil {
					if err = rows.Scan(values...); err == nil {
						if res, ok = value.Interface().(sdk.IUid); ok {
							if !callback(res) {
								break
							}
						} else {
