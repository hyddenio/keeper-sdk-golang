...
func syncDown(v Vault) (result *rebuildTask, err error) {
	var storage = v.VaultStorage()
	rq := &SyncDownCommand{
		Revision:   storage.Revision(),
		Include:    []string{"sfheaders", "sfrecords", "sfusers", "teams", "folders"},
		DeviceId:   auth.DefaultDeviceName,
		DeviceName: auth.DefaultDeviceName,
	...
	if rs.NonSharedData != nil {
		for _, nsd := range rs.NonSharedData {
			...
			encData := nsd.Data()
			if encData != "" {
				if data, err = auth.DecryptAesV1(auth.Base64UrlDecode(encData), v.Auth().AuthContext().DataKey()); err == nil {
					if data, err = auth.EncryptAesV1(data, v.Auth().AuthContext().ClientKey()); err == nil {
						nsd.Data_ = auth.Base64UrlEncode(data)
	...
	if rs.RecordMetaData != nil {
		for _, rmd := range rs.RecordMetaData {
			...
			case 0:
				key = v.Auth().AuthContext().DataKey()
			case 1:
				key, err = auth.DecryptAesV1(auth.Base64UrlDecode(rmd.RecordKey_), v.Auth().AuthContext().DataKey())
			case 2:
				key, err = auth.DecryptRsa(auth.Base64UrlDecode(rmd.RecordKey_), v.Auth().AuthContext().RsaPrivateKey())
			default:
				err = auth.NewKeeperError(fmt.Sprintf("Unsuppoted key type: %d", rmd.RecordKeyType_))
			}
			if key != nil {
				if key, err = auth.EncryptAesV1(key, v.Auth().AuthContext().ClientKey()); err == nil {
					rmd.RecordKey_ = auth.Base64UrlEncode(key)
	...
	if rs.Teams != nil {
		for _, team := range rs.Teams {
			...
			case 1:
				key, err = auth.DecryptAesV1(auth.Base64UrlDecode(team.TeamKey_), v.Auth().AuthContext().DataKey())
			case 2:
				key, err = auth.DecryptRsa(auth.Base64UrlDecode(team.TeamKey_), v.Auth().AuthContext().RsaPrivateKey())
			default:
				err = auth.NewKeeperError(fmt.Sprintf("Unsuppoted key type: %d", team.TeamKeyType_))
			}
			if key != nil {
				var teamKey = key
				if key, err = auth.EncryptAesV1(key, v.Auth().AuthContext().ClientKey()); err == nil {
					team.TeamKey_ = auth.Base64UrlEncode(key)
					...
					if team.SharedFolderKeys != nil {
						...
						for _, sfk := range team.SharedFolderKeys {
							...
							switch sfk.KeyType_ {
							case 1:
								sharedFolderKey = auth.Base64UrlDecode(sfk.SharedFolderKey_)
							case 2:
								if teamPrivateKey == nil {
									if key, err = auth.DecryptAesV1(auth.Base64UrlDecode(team.TeamPrivateKey_), teamKey); err == nil {
										teamPrivateKey, err = auth.LoadRsaPrivateKey(key)
									}
								}
								if teamPrivateKey != nil {
									if key, err = auth.DecryptRsa(auth.Base64UrlDecode(sfk.SharedFolderKey_), teamPrivateKey); err == nil {
										sharedFolderKey, err = auth.EncryptAesV1(key, teamKey)
									}
								}
							}
							if sharedFolderKey != nil {
								sfk.encryptorUid = teamUid
								sfk.SharedFolderKey_ = auth.Base64UrlEncode(sharedFolderKey)
	...
	if rs.SharedFolders != nil {
		for _, sf := range rs.SharedFolders {
			...
			if sf.SharedFolderKey != nil {
				...
				switch *sf.KeyType {
				case 1:
					key, err = auth.DecryptAesV1(auth.Base64UrlDecode(*sf.SharedFolderKey), v.Auth().AuthContext().DataKey())
				case 2:
					key, err = auth.DecryptRsa(auth.Base64UrlDecode(*sf.SharedFolderKey), v.Auth().AuthContext().RsaPrivateKey())
				default:
					err = auth.NewKeeperError(fmt.Sprintf("Unsupported key type: %d", *sf.KeyType))
				}
				if key != nil {
					if key, err = auth.EncryptAesV1(key, v.Auth().AuthContext().ClientKey()); err == nil {
			...
			if sf.Records != nil {
				for _, sfr := range sf.Records {
					...
					result.addRecord(recordUid)
					sfr.encryptorUid = sf.SharedFolderUid_
					storage.RecordKeys().Put(sfr)
				}
			}
			if sf.Teams != nil {
	...
	if rs.UserFolders != nil {
		for _, uf := range rs.UserFolders {
			...
			switch uf.KeyType_ {
			case 1:
				key, err = auth.DecryptAesV1(auth.Base64UrlDecode(uf.UserFolderKey_), v.Auth().AuthContext().DataKey())
			case 2:
				key, err = auth.DecryptRsa(auth.Base64UrlDecode(uf.UserFolderKey_), v.Auth().AuthContext().RsaPrivateKey())
			}
			if key != nil {
				if key, err = auth.EncryptAesV1(key, v.Auth().AuthContext().ClientKey()); err == nil {
					uf.UserFolderKey_ = auth.Base64UrlEncode(key)
...
func (sfr *SyncDownSharedFolderRecord) RecordUid() string {
	return sfr.RecordUid_
}
func (sfr *SyncDownSharedFolderRecord) EncryptorUid() string {
	return sfr.encryptorUid
}
func (sfr *SyncDownSharedFolderRecord) CanShare() bool {
	return sfr.CanShare_
}
func (sfr *SyncDownSharedFolderRecord) CanEdit() bool {
	return sfr.CanEdit_
}
func (sfr *SyncDownSharedFolderRecord) RecordKey() string {
	return sfr.RecordKey_
}
func (sfr *SyncDownSharedFolderRecord) KeyType() int32 {
	return SharedFolderKey
}
func (sfr *SyncDownSharedFolderRecord) SubjectUid() string {
	return sfr.RecordUid()
}
func (sfr *SyncDownSharedFolderRecord) ObjectUid() string {
	return sfr.EncryptorUid()
}
