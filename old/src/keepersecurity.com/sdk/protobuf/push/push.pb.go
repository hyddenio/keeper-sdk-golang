// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.25.0
// 	protoc        v3.10.1
// source: push.proto

package push

import (
	proto "github.com/golang/protobuf/proto"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// This is a compile-time assertion that a sufficiently up-to-date version
// of the legacy proto package is being used.
const _ = proto.ProtoPackageIsVersion4

type MessageType int32

const (
	MessageType_UNKNOWN    MessageType = 0
	MessageType_DNA        MessageType = 1 // to one specific messageSessionUid
	MessageType_SSO        MessageType = 2 // not supported
	MessageType_CHAT       MessageType = 3 // to all user's messageSessionUid except one specific messageSessionUid (origin)
	MessageType_USER       MessageType = 4 // to all user's messageSessionUid except one specific messageSessionUid (origin)
	MessageType_ENTERPRISE MessageType = 5 // to all messageSessionUid for enterpriseId not equal current messageSessionUid
	MessageType_KEEPER     MessageType = 6 // KA or Chat connects to or disconnects from PS
	MessageType_SESSION    MessageType = 7 // to one specific messageSessionUid (somehow different from DNA)
	MessageType_DEVICE     MessageType = 8 // to all devices
	MessageType_TOTP       MessageType = 9 // to 2FA
)

// Enum value maps for MessageType.
var (
	MessageType_name = map[int32]string{
		0: "UNKNOWN",
		1: "DNA",
		2: "SSO",
		3: "CHAT",
		4: "USER",
		5: "ENTERPRISE",
		6: "KEEPER",
		7: "SESSION",
		8: "DEVICE",
		9: "TOTP",
	}
	MessageType_value = map[string]int32{
		"UNKNOWN":    0,
		"DNA":        1,
		"SSO":        2,
		"CHAT":       3,
		"USER":       4,
		"ENTERPRISE": 5,
		"KEEPER":     6,
		"SESSION":    7,
		"DEVICE":     8,
		"TOTP":       9,
	}
)

func (x MessageType) Enum() *MessageType {
	p := new(MessageType)
	*p = x
	return p
}

func (x MessageType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MessageType) Descriptor() protoreflect.EnumDescriptor {
	return file_push_proto_enumTypes[0].Descriptor()
}

func (MessageType) Type() protoreflect.EnumType {
	return &file_push_proto_enumTypes[0]
}

func (x MessageType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MessageType.Descriptor instead.
func (MessageType) EnumDescriptor() ([]byte, []int) {
	return file_push_proto_rawDescGZIP(), []int{0}
}

// KA registers login/logout
type UserRegistrationRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	MessageSessionUid []byte `protobuf:"bytes,1,opt,name=messageSessionUid,proto3" json:"messageSessionUid"`
	UserId            int32  `protobuf:"varint,2,opt,name=userId,proto3" json:"userId"`
	EnterpriseId      int32  `protobuf:"varint,3,opt,name=enterpriseId,proto3" json:"enterpriseId"`
}

func (x *UserRegistrationRequest) Reset() {
	*x = UserRegistrationRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_push_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UserRegistrationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserRegistrationRequest) ProtoMessage() {}

func (x *UserRegistrationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_push_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserRegistrationRequest.ProtoReflect.Descriptor instead.
func (*UserRegistrationRequest) Descriptor() ([]byte, []int) {
	return file_push_proto_rawDescGZIP(), []int{0}
}

func (x *UserRegistrationRequest) GetMessageSessionUid() []byte {
	if x != nil {
		return x.MessageSessionUid
	}
	return nil
}

func (x *UserRegistrationRequest) GetUserId() int32 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *UserRegistrationRequest) GetEnterpriseId() int32 {
	if x != nil {
		return x.EnterpriseId
	}
	return 0
}

// KA sends ws message
type KAToPushServerRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	MessageType          MessageType `protobuf:"varint,1,opt,name=messageType,proto3,enum=Push.MessageType" json:"messageType"`
	Message              string      `protobuf:"bytes,2,opt,name=message,proto3" json:"message"`
	MessageSessionUid    []byte      `protobuf:"bytes,3,opt,name=messageSessionUid,proto3" json:"messageSessionUid"`
	EncryptedDeviceToken [][]byte    `protobuf:"bytes,4,rep,name=encryptedDeviceToken,proto3" json:"encryptedDeviceToken"` // aka platformDeviceToken
	UserId               []int32     `protobuf:"varint,5,rep,packed,name=userId,proto3" json:"userId"`
	EnterpriseId         []int32     `protobuf:"varint,6,rep,packed,name=enterpriseId,proto3" json:"enterpriseId"`
}

func (x *KAToPushServerRequest) Reset() {
	*x = KAToPushServerRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_push_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *KAToPushServerRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*KAToPushServerRequest) ProtoMessage() {}

func (x *KAToPushServerRequest) ProtoReflect() protoreflect.Message {
	mi := &file_push_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use KAToPushServerRequest.ProtoReflect.Descriptor instead.
func (*KAToPushServerRequest) Descriptor() ([]byte, []int) {
	return file_push_proto_rawDescGZIP(), []int{1}
}

func (x *KAToPushServerRequest) GetMessageType() MessageType {
	if x != nil {
		return x.MessageType
	}
	return MessageType_UNKNOWN
}

func (x *KAToPushServerRequest) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *KAToPushServerRequest) GetMessageSessionUid() []byte {
	if x != nil {
		return x.MessageSessionUid
	}
	return nil
}

func (x *KAToPushServerRequest) GetEncryptedDeviceToken() [][]byte {
	if x != nil {
		return x.EncryptedDeviceToken
	}
	return nil
}

func (x *KAToPushServerRequest) GetUserId() []int32 {
	if x != nil {
		return x.UserId
	}
	return nil
}

func (x *KAToPushServerRequest) GetEnterpriseId() []int32 {
	if x != nil {
		return x.EnterpriseId
	}
	return nil
}

//Client sends to push
type WssConnectionRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	MessageSessionUid    []byte `protobuf:"bytes,1,opt,name=messageSessionUid,proto3" json:"messageSessionUid"`
	EncryptedDeviceToken []byte `protobuf:"bytes,2,opt,name=encryptedDeviceToken,proto3" json:"encryptedDeviceToken"` // aka platformDeviceToken
	DeviceTimeStamp      int64  `protobuf:"varint,3,opt,name=deviceTimeStamp,proto3" json:"deviceTimeStamp"`
}

func (x *WssConnectionRequest) Reset() {
	*x = WssConnectionRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_push_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WssConnectionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WssConnectionRequest) ProtoMessage() {}

func (x *WssConnectionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_push_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WssConnectionRequest.ProtoReflect.Descriptor instead.
func (*WssConnectionRequest) Descriptor() ([]byte, []int) {
	return file_push_proto_rawDescGZIP(), []int{2}
}

func (x *WssConnectionRequest) GetMessageSessionUid() []byte {
	if x != nil {
		return x.MessageSessionUid
	}
	return nil
}

func (x *WssConnectionRequest) GetEncryptedDeviceToken() []byte {
	if x != nil {
		return x.EncryptedDeviceToken
	}
	return nil
}

func (x *WssConnectionRequest) GetDeviceTimeStamp() int64 {
	if x != nil {
		return x.DeviceTimeStamp
	}
	return 0
}

//PS sends to clients
type WssClientResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	MessageType MessageType `protobuf:"varint,1,opt,name=messageType,proto3,enum=Push.MessageType" json:"messageType"`
	Message     string      `protobuf:"bytes,2,opt,name=message,proto3" json:"message"`
}

func (x *WssClientResponse) Reset() {
	*x = WssClientResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_push_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WssClientResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WssClientResponse) ProtoMessage() {}

func (x *WssClientResponse) ProtoReflect() protoreflect.Message {
	mi := &file_push_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WssClientResponse.ProtoReflect.Descriptor instead.
func (*WssClientResponse) Descriptor() ([]byte, []int) {
	return file_push_proto_rawDescGZIP(), []int{3}
}

func (x *WssClientResponse) GetMessageType() MessageType {
	if x != nil {
		return x.MessageType
	}
	return MessageType_UNKNOWN
}

func (x *WssClientResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

type PushServerDeviceRegistrationRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	EncryptedDeviceToken []byte `protobuf:"bytes,1,opt,name=encryptedDeviceToken,proto3" json:"encryptedDeviceToken"`
	PushToken            string `protobuf:"bytes,2,opt,name=pushToken,proto3" json:"pushToken"`
	MobilePushPlatform   string `protobuf:"bytes,3,opt,name=mobilePushPlatform,proto3" json:"mobilePushPlatform"`
	TransmissionKey      []byte `protobuf:"bytes,4,opt,name=transmissionKey,proto3" json:"transmissionKey"`
}

func (x *PushServerDeviceRegistrationRequest) Reset() {
	*x = PushServerDeviceRegistrationRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_push_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *PushServerDeviceRegistrationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PushServerDeviceRegistrationRequest) ProtoMessage() {}

func (x *PushServerDeviceRegistrationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_push_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PushServerDeviceRegistrationRequest.ProtoReflect.Descriptor instead.
func (*PushServerDeviceRegistrationRequest) Descriptor() ([]byte, []int) {
	return file_push_proto_rawDescGZIP(), []int{4}
}

func (x *PushServerDeviceRegistrationRequest) GetEncryptedDeviceToken() []byte {
	if x != nil {
		return x.EncryptedDeviceToken
	}
	return nil
}

func (x *PushServerDeviceRegistrationRequest) GetPushToken() string {
	if x != nil {
		return x.PushToken
	}
	return ""
}

func (x *PushServerDeviceRegistrationRequest) GetMobilePushPlatform() string {
	if x != nil {
		return x.MobilePushPlatform
	}
	return ""
}

func (x *PushServerDeviceRegistrationRequest) GetTransmissionKey() []byte {
	if x != nil {
		return x.TransmissionKey
	}
	return nil
}

type SnsMessage struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	MessageType MessageType `protobuf:"varint,1,opt,name=messageType,proto3,enum=Push.MessageType" json:"messageType"`
	Message     []byte      `protobuf:"bytes,2,opt,name=message,proto3" json:"message"`
}

func (x *SnsMessage) Reset() {
	*x = SnsMessage{}
	if protoimpl.UnsafeEnabled {
		mi := &file_push_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *SnsMessage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SnsMessage) ProtoMessage() {}

func (x *SnsMessage) ProtoReflect() protoreflect.Message {
	mi := &file_push_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SnsMessage.ProtoReflect.Descriptor instead.
func (*SnsMessage) Descriptor() ([]byte, []int) {
	return file_push_proto_rawDescGZIP(), []int{5}
}

func (x *SnsMessage) GetMessageType() MessageType {
	if x != nil {
		return x.MessageType
	}
	return MessageType_UNKNOWN
}

func (x *SnsMessage) GetMessage() []byte {
	if x != nil {
		return x.Message
	}
	return nil
}

var File_push_proto protoreflect.FileDescriptor

var file_push_proto_rawDesc = []byte{
	,
}

var (
	file_push_proto_rawDescOnce sync.Once
	file_push_proto_rawDescData = file_push_proto_rawDesc
)

func file_push_proto_rawDescGZIP() []byte {
	file_push_proto_rawDescOnce.Do(func() {
		file_push_proto_rawDescData = protoimpl.X.CompressGZIP(file_push_proto_rawDescData)
	})
	return file_push_proto_rawDescData
}

var file_push_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_push_proto_msgTypes = make([]protoimpl.MessageInfo, 6)
var file_push_proto_goTypes = []interface{}{
	(MessageType)(0),                            // 0: Push.MessageType
	(*UserRegistrationRequest)(nil),             // 1: Push.UserRegistrationRequest
	(*KAToPushServerRequest)(nil),               // 2: Push.KAToPushServerRequest
	(*WssConnectionRequest)(nil),                // 3: Push.WssConnectionRequest
	(*WssClientResponse)(nil),                   // 4: Push.WssClientResponse
	(*PushServerDeviceRegistrationRequest)(nil), // 5: Push.PushServerDeviceRegistrationRequest
	(*SnsMessage)(nil),                          // 6: Push.SnsMessage
}
var file_push_proto_depIdxs = []int32{
	0, // 0: Push.KAToPushServerRequest.messageType:type_name -> Push.MessageType
	0, // 1: Push.WssClientResponse.messageType:type_name -> Push.MessageType
	0, // 2: Push.SnsMessage.messageType:type_name -> Push.MessageType
	3, // [3:3] is the sub-list for method output_type
	3, // [3:3] is the sub-list for method input_type
	3, // [3:3] is the sub-list for extension type_name
	3, // [3:3] is the sub-list for extension extendee
	0, // [0:3] is the sub-list for field type_name
}

func init() { file_push_proto_init() }
func file_push_proto_init() {
	if File_push_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_push_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UserRegistrationRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_push_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*KAToPushServerRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_push_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*WssConnectionRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_push_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*WssClientResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_push_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*PushServerDeviceRegistrationRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_push_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*SnsMessage); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_push_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   6,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_push_proto_goTypes,
		DependencyIndexes: file_push_proto_depIdxs,
		EnumInfos:         file_push_proto_enumTypes,
		MessageInfos:      file_push_proto_msgTypes,
	}.Build()
	File_push_proto = out.File
	file_push_proto_rawDesc = nil
	file_push_proto_goTypes = nil
	file_push_proto_depIdxs = nil
}
