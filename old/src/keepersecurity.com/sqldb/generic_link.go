...
func (s *sqlLinkStorage) PutLink(link vault.IUidLink) {
...
func (s *sqlLinkStorage) GetLink(subjectUid string, objectUid string) (result vault.IUidLink) {
	fields := s.linkInfo.GetDataFields()
	...
	if stmt, err = s.db.GetStatement(query); err == nil {
		...
		if values, err = GetScanner(value.Elem(), keyValues, dataValues); err == nil {
			if err = row.Scan(values...); err == nil {
				if result, ok = value.Interface().(vault.IUidLink); !ok {
...
func (s *sqlLinkStorage) GetLinksForSubject(subjectUid string, callback func (vault.IUidLink) bool) {
			...
			for rows.Next() {
				...
				if values, err = GetScanner(value.Elem(), keyValues, dataValues); err == nil {
					if err = rows.Scan(values...); err == nil {
						var link vault.IUidLink
						if link, ok = value.Interface().(vault.IUidLink); ok {
							if !callback(link) {
								break
							}
						} else {
...
func (s *sqlLinkStorage) GetLinksForObject(objectUid string, callback func (vault.IUidLink) bool) {
...
func (s *sqlLinkStorage) GetAllLinks(callback func (vault.IUidLink) bool) {
